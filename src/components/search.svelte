	...
	<div class="relative" use:clickOutside on:outclick={clearQuery}>
		<div class="relative truncate">
			...
			<input
				...
				name="search"
				class={twMerge(
					`block w-full pl-10 pr-3 py-2 rounded-md leading-5 bg-gray-50/80 focus:bg-white/80 caret-gray-400 focus:outline-none focus:ring-1 focus:ring-transparent border border-gray-300 focus:border-gray-400 sm:text-sm ${clazz}`
				)}
				type="text"
				autocomplete="off"
				bind:value={query}
