 ...
 */
function uniformity(values: number[]): number {
...
function loadHistoricalData(dirPath: string) {
	...
	return timepoints
		...
		.map((timestamp) => ({
			timestamp,
			...yaml.parse(fs.readFileSync(`${dirPath}/${timestamp}.yml`, 'utf-8'))
		}));
}

function loadAssetData(assetId: string) {
...
async function followBackingTree(
	id: string,
	backedAsset: object, //TODO better typing
	assetValue: number,
	assets: { [key: string]: object }, //TODO better typing
	level: number,
	timestamp: string
): Promise<object[]> {
	if (!backedAsset) {
	...
	const backedAssets = Object.entries(closest(backedAsset.backing, timestamp).assets)
		...
		.map(([key, value]) => [
			key,
			value,
			value * closest(assets[key].price, timestamp).usd,
			assets[key]
		]);

	const totalBackingUsd = backedAssets.reduce(
		(partialSum, [key, value, backingUsd]) => partialSum + backingUsd,
		0
	);
...
async function buildBackingTree(id: string, asset, assets, timestamp: string) {
	const supply =
		closest(asset.supply, timestamp).circulating || closest(asset.supply, timestamp).total || 0;
	...
	for (const link of links) {
		const existing_link = reduced_links.find(
...
export async function prepareData() {
	...
	const assets = assetNames.reduce(
		(a, v) => ({
			...a,
			[v]: { ...loadAssetData(v), icon: v in icons ? icons[v].href : undefined }
		}),
		{}
	);

	const tokenAssetMapping = Object.entries(assets).reduce(
		(a, [k, v]) => ({ ...a, ...Object.keys(v.contracts).reduce((a, c) => ({ ...a, [c]: k }), {}) }),
		{}
	);
	...
	let globalBacking = Object.values(assets).reduce(
		(a, { backing }) => {
